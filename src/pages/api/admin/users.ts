...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const isAdmin = adminPositions.includes(currentUser.position?.toLowerCase() || "");
    ...
    // Create a map of user emails who logged in today
    const loggedInTodayEmails = new Set(
      attendanceRecords.map(record => record.userEmail)
    );

    // Get the latest login time for each user from attendance records
    const latestLogins = await db.collection("attendance").aggregate([
      {
        $group: {
          _id: "$userEmail",
    ...
    const usersWithLoginStatus = users.map(user => ({
      ...
      lastLoginToday: loggedInTodayEmails.has(user.email),
